...
int main()
{
    ...
    void **dictionary = (void **)malloc(TOTAL_ALLOCS * sizeof(void *));
    int *dictionary_elem_size = (int *)malloc(TOTAL_ALLOCS * sizeof(int));
    int dictionary_ct = 0;
    int data_written = 0;
    ...
    for (i = 0; i < TOTAL_ALLOCS; i++)
    {
        ...
        else
        {
            ...
            {
                *((void **)((char *)ptr + size - sizeof(void *))) = &dictionary[dictionary_ct];
    ...
    for (i = dictionary_ct - 1; i >= 0; i--)
    {
        ...
        if (*((void **)((char *)dictionary[i] + dictionary_elem_size[i] - sizeof(void *))) != &dictionary[i])
        {
            printf("Memory failed to contain correct data after many allocations (end of segment)!\n");
            return 101;
        }

        char *memory_check = (char *)dictionary[i] + sizeof(void *);
        char *memory_check_end = (char *)dictionary[i] + dictionary_elem_size[i] - sizeof(void *) - 1;
